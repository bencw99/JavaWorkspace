...
public class Polygon3D
{
	...
	public PolygonProjection getProjection(View view)
	{
		...
		Point3D viewedPoint = view.getViewedPoint();
		...
		/* Initial translation of space to position rotation axis at origin */
		...
		for(int i = 0; i < points.length; i ++)
		{
			double perspectiveMod = View.EYE_PLANE_DIST/(viewPoint.getZ() - rotatedPoints[i].getZ() + View.EYE_PLANE_DIST);
			projectedPoints[i] = new Point2D(perspectiveMod*rotatedPoints[i].getX(), perspectiveMod*rotatedPoints[i].getY());
		}
		
		return (new PolygonProjection(projectedPoints, 0, 0, color)).rotate(new Point2D(viewPoint.getX(), viewPoint.getY()), view.getTurnAngle());
	}
